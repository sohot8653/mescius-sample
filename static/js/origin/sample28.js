...
window.onload = function() {
    ...
    $('#btnEnroll').on('click', function(e) {
        location.href = `/sample?depth1=${depth1}&depth2=はいそう うちわけしょ&sampleId=29`;
    });

    $('#btnTemplate').on('click', function(e) {
        location.href = `/sample?depth1=${depth1}&depth2=はいそう うちわけしょ&sampleId=30`;
    });
};

function initSpread(spread) {
    fetch('json/Sample28.ssjson')
    .then(response => {
    ...
    .then(x => {
        ...
        if(localStorage.getItem('sample30Index') != null) {
            var idx = Number(localStorage.getItem('sample30Index'));

            for(var i = 0; i < idx + 1; i++) {
                dataAdd.push({
                    c1: localStorage.getItem('sample30Num' + i),
                    c2: 'TESTER',
                    c3: getToday(),
                    c4: i
                })
                ...
                switch (cellType._text) {
                    case '詳細':
                        if(sheet.getCell(row, col + 1).text() != '') {
                            location.href = `/sample?depth1=${depth1}&depth2=はいそう うちわけしょ&sampleId=31&sample30Index=`+sheet.getCell(row, col + 1).text();
                        } else {
                            location.href = `/sample?depth1=${depth1}&depth2=はいそう うちわけしょ&sampleId=31`;
                        }
                        break;
                }
            }
    ...
    function drawRow(startRow, spread, sheet, SheetArea, data) {
        ...
        sheet.setCellType(startRow, 3, new GC.Spread.Sheets.CellTypes.Button().text("詳細"), GC.Spread.Sheets.SheetArea.viewport);
...
function getData() {
  ...
  for(var i = 1; i < 60; i++) {
    ...
    obj.c2 = getRandomArrayElement(['KIM', 'KANG', 'KO', 'PARK0', 'YUN', 'LEE']);
    obj.c3 = '2024-01-29';
