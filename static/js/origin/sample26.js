...
function formatDayCell(sender, e) {
  ...
  let tip = wijmo.format('<div class="event-tip event-type-{eventType}">' +
                         ...
                         '</div>', {
    date: e.data,
    eventMessage: event ? event.msg : '日程なし',
...
// 지금과 1년 전 사이에 이벤트 생성
function getEvents() {
let arr = [], types = ['Maintenance', 'Incident', 'Notice', 'Outage'], messages = ['事業や企業の主体1/地域1/腰掛け便器', '事業や企業の主体2/地域2/床施工'];
//
//for (let i = 0; i < 120; i++) {
...
function initGanttSheetWithIdParentIdData(spread) {
  ...
  var myTable1 = dataManager.addTable("myTable1", {
      ...
      remote: {
          read: {
              url: 'json/Sample5_Gantt_Id_jp.json'
          }
      },
      schema: {
          hierarchy: {
  ...
  var ganttSheet = spread.addSheetTab(0, "施工スケジュール", GC.Spread.Sheets.SheetType.ganttSheet);
  var view = myTable1.addView("myView1", [
      ...
      { value: "name", caption: "作業名", width: 200, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "duration", caption: "作業期間", width: 90, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "resource", caption: "資源量", width: 80, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "cost", caption: "費用", width: 100, style: { formatter: "0¥" }, headerStyle: {backColor : "#36495e", foreColor:"white"}},
