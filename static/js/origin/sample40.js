...
function formatDayCell(sender, e) {
  ...
  let tip = wijmo.format('<div class="event-tip event-type-{eventType}">' +
                         ...
                         '</div>', {
    date: e.data,
    eventMessage: event ? event.msg : 'No schedule',
...
// 지금과 1년 전 사이에 이벤트 생성
function getEvents() {
let arr = [], types = ['Maintenance', 'Incident', 'Notice', 'Outage'], messages = ['Construction1', 'Construction2'];
//
//for (let i = 0; i < 120; i++) {
...
function initGanttSheetWithIdParentIdData(spread) {
  ...
  var myTable1 = dataManager.addTable("myTable1", {
      ...
      remote: {
          read: {
              url: 'json/Sample5_Gantt_Id_en.json'
          }
      },
      schema: {
          hierarchy: {
  ...
  var ganttSheet = spread.addSheetTab(0, "Construction schedule", GC.Spread.Sheets.SheetType.ganttSheet);
  var view = myTable1.addView("myView1", [
      ...
      { value: "name", caption: "name", width: 200, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "duration", caption: "duration", width: 90, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "resource", caption: "resource", width: 80, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: "cost", caption: "cost", width: 100, style: { formatter: "$0" }, headerStyle: {backColor : "#36495e", foreColor:"white"}},
      { value: '=IF([@cost]="","",[@cost]/[@resource])', caption: "AVG Cost", width: 100, style: { formatter: "$0" }, headerStyle: {backColor : "#36495e", foreColor:"white"}}
  ]);
