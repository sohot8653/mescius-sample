...
window.onload = function () {
  ...
  if (
    ...
    fetch("json/Sample48.ssjson")
      ...
      .then((x) => {
        $("#txtTemplateName").text(localStorage.getItem("templateName_en"));
  ...
  $("#btnSave").on("click", function (e) {
    ...
    localStorage.setItem("templateName_en", $("#txtName").val());
    localStorage.setItem("templateJson_en", jsonStr);
    location.reload(true);
  });

  $("#btnGrid").on("click", function (e) {
    location.href = `/sample?depth1=${depth1}&depth2=Delivery statement&sampleId=47`;
  });
};
...
function initDefaultOptions() {
  ...
  container.appendChild(
    initOptions(importSSJsonOptions, FileType.SSJson, mode)
  );
  ...
  container.appendChild(
    initOptions(exportSSJsonOptions, FileType.SSJson, mode)
  );
...
function getOptions(mode) {
  ...
  var options = {};
  var props =
    mode === "open" ? openOptionsDict[fileType] : saveOptionsDict[fileType];
  container = container.querySelector("." + fileType);

  props.forEach(function (prop) {
    var element = container.querySelector(
      "#" + getElementId(mode, fileType, prop.propName)
    );
    var value;
    if (prop.type === "boolean") {
