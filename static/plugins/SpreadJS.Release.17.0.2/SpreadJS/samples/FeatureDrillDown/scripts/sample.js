...
var isFirstBlur = true; // when enter key down, will blur, and will run this function again.
function updateStringProperty(e) {
    if (!isFirstBlur) {
        return;
    }
    if (e.keyCode !== undefined && e.keyCode !== 13) { // not enter and not blur, just input normal value, return
        return;
    }
    var $element = $(this),
        $parent = $element.parent(),
        name = $parent.data("name"),
        value = $element.val();
    if (e.keyCode === 13) { // if is enter key, blur.
        isFirstBlur = false;
        $element.blur();
        isFirstBlur = true;
    }
...
function doPrepareWork() {
    ...
    function addEventHandlers() {
        ...
        $(".insp-text input.editor").keyup(updateStringProperty);
    }
...
function setCellContent() {
    ...
    sheet.getCell(row, ++col).cellType(cellType).value("http://developer.mescius.com/spreadjs/");
...
function setDataContent() {
    ...
    dateValidator.highlightStyle({
        ...
        image:"./css/images/icon-for-app.ico"
    });
...
function attachSpreadEvents(rebind) {
    ...
    spread.bind(spreadNS.Events.ActiveSheetChanged, function () {
        ...
        var sheet = spread.getActiveSheet(),
            ...
            chart,
            shape;
        var slicers = sheet.slicers && sheet.slicers.all();
        ...
        if (sheet.getSelections().length === 0) {
            ...
            sheet.shapes.all().forEach(function (sha) {
                if (!shape && sha.isSelected() && !isChart(sha)) {
                    shape = sha;
                }
            });
...
function importSpreadFromSJS(file) {
...
function setFormatter(value, targetElement) {
    var sheet = spread.getActiveSheet();
    execInSelections(sheet, "formatter", function (sheet, row, column) {
        ...
        var formatString = value && value.trim(), isValidFormat = true;
...
function applyChartTitle() {
    ...
    if(chart){
        ...
        var title = chart.title() || {}; //Fix SJS-560 that title maybe null.
...
function setGroupShapeCommonAttrs(groupShape) {
    setTextValue("shapeName", groupShape.name());
    setCheckValue("allowShapeMove", groupShape.allowMove());
    setCheckValue("allowShapeResize", groupShape.allowResize());
    setCheckValue("shapeCanPrint", groupShape.canPrint());
    setCheckValue("shapeIsVisible", groupShape.isVisible());
    setCheckValue("shapeDynamicMove", groupShape.dynamicMove());
    setCheckValue("shapeDynamicSize", groupShape.dynamicSize());
    setCheckValue("shapeIsLocked", groupShape.isLocked());
    setCheckValue("shpaeIsSelected", groupShape.isSelected());
    setTextValue("shapeRotate", Number((groupShape.rotate&&groupShape.rotate())||0));
}

function setGroupShapeWidthHeight(groupShape) {
    setNumberValue("baseShapeWidth", groupShape.width());
    setNumberValue("baseShapeHeight", groupShape.height());
}
...
function updateShapeSetting(shapes) {
    ...
    var _setNormalShape = function(shape, shapeStyle) { // width height align fontSize fontFamily text color 
    ...
    var _digShapes = function(_shapes) {
        _shapes.forEach(function(shape){
            ...
            if(shape && shapeType === "shapeGroup"){
                ...
                // fix bug SJS-599, change shape group size doesn't take effect.
                setGroupShapeCommonAttrs(shape);
                setGroupShapeWidthHeight(shape);
            } else {
    ...
    var _applayShapeSettingToItem = function (_shapes, deep) {
        _shapes.forEach(function(item) {
            ...
            if(itemType === 'shapeGroup') {
                ...
                item.rotate(rotate);
                setGroupShapeCommonAttrs(item);
                setGroupShapeWidthHeight(item);
            } else {
...
function isChart (chart) {
    return chart instanceof spreadNS.Charts.Chart;
}

function isSlicer (slicer) {
    return slicer.drawingType === 'Slicer' || slicer.drawingType === 'Timeline';
}